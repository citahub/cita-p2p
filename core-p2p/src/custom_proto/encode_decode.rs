...
impl Encoder for Codec {
    ...
    fn encode(&mut self, msg: Self::Item, buf: &mut BytesMut) -> io::Result<()> {
        encode_to_network(buf, msg);
        Ok(())
    }
}

pub fn encode_to_network(buf: &mut BytesMut, msg: Option<Vec<u8>>) {
    let mut request_id_bytes = [0; 8];

    if let Some(body) = msg {
        let length_full = body.len();
        if length_full > u32::max_value() as usize {
            error!("The message is too long {}.", body.len());
            return;
        }
...
pub fn decode_network_message(buf: &mut BytesMut) -> Option<Vec<u8>> {
    if buf.len() < 8 {
        return None;
    }
...
#[cfg(test)]
mod test {
    use super::{decode_network_message, encode_to_network};
    use bytes::BytesMut;

    #[test]
    fn convert_empty_message() {
        let mut buf = BytesMut::with_capacity(4 + 4);
        encode_to_network(&mut buf, None);
        let pub_msg_opt = decode_network_message(&mut buf);
    ...
    fn convert_messages() {
        ...
        encode_to_network(&mut buf, Some(msg.clone()));
        let pub_msg_opt = decode_network_message(&mut buf);
